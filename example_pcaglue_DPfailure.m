...
for jjj=[248]
    ...
    figure;plot(-1*ax00(:,122)','LineWidth',4),set(gca,'fontsize',60);xlabel('Samples in the RF line','FontSize',60); ylabel('Displacement in pixels','FontSize',60);
    axis([1 2224 0 6])
    
    %In this part, we try to change the estimated some estimates of the DP
    ax00(1179:end,75:199)=-2;
    ax00(1179:end,122+15)=-4;
    ...
    strainA_10(strainA_10<1.6e-3) = 1.6e-3;
    ...
    strainA(strainA<1.6e-3) = 1.6e-3;
    strainA(strainA>0.0059)= 0.0059;
    startA = 1; endA = size(strainA,2);
    startRF = 1; endRF = size(strainA,1); 
    figure;
    imagesc([0 xRat*(endA-startA)],yRat*[startRF endRF],strainA); set(gca,'fontsize',60);
    colorbar;
